...
@app.get("/addresses/{anumaAddress}/transactions",
         response_model=TransactionForAddressResponse,
         response_model_exclude_unset=True,
         tags=["Anuma addresses"])
@sql_db_only
async def get_transactions_for_address(
        anumaAddress: str = Path(
            description="Anuma address as string e.g. "
                        f"{ADDRESS_EXAMPLE}",
    ...
    #
    # WHERE "script_public_key_address" = 'anuma:qp7d7rzrj34s2k3qlxmguuerfh2qmjafc399lj6606fc7s69l84h7mrj49hu6'
    #
    # ORDER by transactions_outputs.transaction_id
    anumaAddress = re.sub(r"^anuma(test)?:", "",
                          anumaAddress)  # Custom query bypasses the TypeDecorator, must handle it manually
    async with async_session() as session:
        resp = await session.execute(
            ...
            WHERE o.script_public_key_address = :anumaAddress
            ORDER by t.block_time DESC
            LIMIT 500"""),
            {'anumaAddress': anumaAddress})
...
@app.get("/addresses/{anumaAddress}/full-transactions",
...
async def get_full_transactions_for_address(
    ...
    async with async_session() as s:
        ...
        tx_within_limit_offset = await s.execute(select(TxAddrMapping.transaction_id)
                                                 .filter(TxAddrMapping.address == anumaAddress)
                                                 .limit(limit)
                                                 .offset(offset)
...
@app.get("/addresses/{anumaAddress}/full-transactions-page",
         response_model=List[TxModel],
         response_model_exclude_unset=True,
         tags=["Anuma addresses"])
@sql_db_only
...
@app.get("/addresses/{anumaAddress}/transactions-count",
...
async def get_transaction_count_for_address(
    ...
    async with async_session() as s:
        count_query = select(func.count()).filter(TxAddrMapping.address == anumaAddress)
...
@app.get("/addresses/{anumaAddress}/name",
         response_model=AddressName | None,
         tags=["Anuma addresses"])
@sql_db_only
async def get_name_for_address(
        response: Response,
        anumaAddress: str = Path(description="Anuma address as string e.g. "
                                             "anuma:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e",
                                 regex=REGEX_ANUMA_ADDRESS)
):
    """
    ...
    async with async_session() as s:
        r = (await s.execute(
            select(AddressKnown)
            .filter(AddressKnown.address == anumaAddress))).first()
