...
class AnumadInfoResponse(BaseModel):
    mempoolSize: str = "1"
...
@app.get("/info/anumad", response_model=AnumadInfoResponse, tags=["Anuma network info"])
async def get_anumad_info():
    """
    Get some information for anumad instance, which is currently connected.
    """
    resp = await anumad_client.request("getInfoRequest")
