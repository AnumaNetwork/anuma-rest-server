...
from anumad.AnumadThread import AnumadThread


# pipenv run python -m grpc_tools.protoc -I./protos --python_out=. --grpc_python_out=. ./protos/rpc.proto ./protos/messages.proto ./protos/p2p.proto

class AnumadClient(object):
    def __init__(self, anumad_host, anumad_port):
    ...
    async def request(self, command, params=None, timeout=5):
        with AnumadThread(self.anumad_host, self.anumad_port) as t:
            return await t.request(command, params, wait_for_response=True, timeout=timeout)

    async def notify(self, command, params, callback):
        t = AnumadThread(self.anumad_host, self.anumad_port, async_thread=True)
